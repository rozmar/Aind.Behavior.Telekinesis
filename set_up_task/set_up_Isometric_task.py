...
def mock_rig() -> AindTelekinesisRig:
    ...
    return AindTelekinesisRig(
        ...
        harp_clock_generator=rig.harp.HarpTimestampGeneratorGen3(port_name="COM5"),#doneHarpWhiteRabbit
        harp_load_cells=lcc.LoadCells(port_name="COM4", calibration=load_cells_calibration),#done
        harp_analog_input=None,
        manipulator=AindManipulatorDevice(port_name="COM7", calibration=manipulator_calibration),#done
...
def mock_task_logic() -> tl.AindTelekinesisTaskLogic:
    trial_prototype = tl.Trial(
        ...
        action_source_0 = tl.LoadCellActionSource(channel=2),
        action_source_1 = tl.LoadCellActionSource(channel=3),
...
ap_range = [-100,100] # these are anterior-posterior load cell readings
